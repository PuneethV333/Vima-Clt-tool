...
const StartStep = () => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-base sm:text-lg font-semibold">Step 1: Common for all steps</h1>
        <p className="text-zinc-400 text-sm sm:text-base">Install the CLI package globally using npm.</p>
      </div>
      ...
      <div className="bg-zinc-950 border border-zinc-700 rounded-xl overflow-hidden">
        <div className="px-4 py-2 border-b border-zinc-700 text-zinc-400 text-sm flex justify-between items-center">
          ...
          <button
            onClick={handleCopy}
            className="p-1 sm:p-2 rounded hover:bg-zinc-800 text-zinc-300 flex items-center gap-1"
            title="Copy"
          >
            {copied ? (
              <>
